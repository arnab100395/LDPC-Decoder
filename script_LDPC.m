...
max_num_frame = 1000;
% %quantized = 1;
% disp('Press 1 for quantized, any other number for non quantized')
quantized = input('Press 1 for quantized LDPC Decoder, any other number for non quantized LDPC decoder \n');
SNR = 1:0.4:4.2;
frac_range = [3];
...
for SNR_index = 1:length(SNR);%--- Max. LLR For Output of Detector
    ...
    for frame_index = 1:max_num_frame
        ...
        %---------------Receiver Decoding--------------------------------------
        if (quantized == 1)    
            % TODO: process llr if you want run quantized version
             % Output = ldpc_decoder_quantized(llr, H);
        else 
             %%
